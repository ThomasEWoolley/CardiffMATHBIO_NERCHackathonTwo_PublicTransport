...
%based on shield length 2R, shield gradient Delta,find the endpoints for a
%single shield by performing

%x = center +- sqrt(R^2/(1+delta^2)), y = delta*x
delta = 0.5*ones(76,1);
R = 0.5*ones(76,1);

shield_endpoints = zeros(76,4);
for i = 1:76
 x = sqrt(R(i)^2/(1+delta(i)^2));
 y = delta(i)*x;
 shield_endpoints(i,:) = [shield_locations(i,1)-x shield_locations(i,1)+x shield_locations(i,2)-y shield_locations(i,2)+y]
end
    

csvwrite('shield_locations_diagonal.csv',shield_endpoints)
clf
for i = 1:76
    hold on
